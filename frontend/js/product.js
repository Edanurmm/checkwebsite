...
async function createResultGridItems(categoryId, freeShipping, freeReturn){
    ...
    for (const key in products){
        ...
        if(productImage != "" && productColor != ""){  //iphone14 olsa .sız split edince null array ve olmayan arrayın 0. elemanı kodu patlatır split.length>0
        ...
        img.src = "images/" + productImage;  //o teklifin bir rengi varsa !! hata kontrol   o renkteki resmi göstermesi için 
        ...
        let offers = await getOffers(products[key].productId, freeShipping, freeReturn); //ürünle ilgili tüm tekliflerinı aılıyoruz pid ile

        var colorArr = [];  //ürünün tüm tekliflerine gidip renklerine ulaşmak
        for(const key in offers){  //diğer tekliflerde farklı renkler var mı? ürünün tüm renklerine ulaşmak gerek ürünün tüm tekliflerini aldık offersta dönüyoruz 5 teklif 3ü siyah 2si sarı 1 siyah 1 sarı dönsün istiyorum yuvarlaklar için 

            var colorVariant = await getProductDetailAttributesKeyValue(offers[key].productDetailId, "Color"); //her teklifin pdi'siyle color'ını alıyoruz

            if(colorVariant != "" && !colorArr.includes(colorVariant))  //renk atribute yoksa boşluk dönyor
            {
                colorArr.push(colorVariant); //color rengine mesela varsa siyahı ekle tekrar dön 2. teklifi aldık siyah mesela false döner başına çıkar 3e geldik sarı yoktu şimdi ekledik 
            }
        }        

        for(const index in colorArr)  //dizinin içinde döngüyle dön ekrandaki o yuvarlakları oluştur 
        {
            var divImageVariant = document.createElement("div");  //dıştaki div
            divImageVariant.className = "image-variant";      //içteki div
            ...
            divImageVariantImage.style.backgroundImage ="url(images/" + products[key].image.split(".")[0] + "_" + colorArr[index] + "." + products[key].image.split(".")[1];  //1. varyantın rengi oluştu siyah sonra da sarı mesela
